...
let notify = {
  ...
  insert: {
    css(css, id, currentScript, selector) {
      if(css && id){
  ...
  notify(params) {
    if(params){
      ...
      if(message || !message && style){
        this.insert.css(css, 'notify', false, selector)
      }
      //function notifyClose
      function notifyClose(e){
        var _notify = e.closest('.new_notify')
      ...
      if(ok){
        ...
        if(all_notify.querySelectorAll(`div.new_notify`).length >= 7){
          setTimeout(() => {
            notifyClose(all_notify.querySelector(`div.new_notify`))
          }, 200);
        }
        //btnClose
        var close = false;
        ...
        if(autoClose){
          setTimeout(() => {
            if(!close){
              notifyClose(selector.getElementById(ID))
            }
...
astronaut='anything';astronaut=original;console.log('is var')}catch(err){console.log(`%cIt looks like you already have the full astronaut library in your project, to avoid mistakes, if you are not using the full library and you only want to use a specific library, remove the full library! 🤔 `,` color: #ff8080;background-color: #290000;padding: 0.3rem 1.8rem 0.3rem 0.3rem;
    ...
    `);notify=howtouse}}catch(error){if(typeof astronaut==='undefined'){window.astronaut=notify
notify=howtouse}else{astronaut=Object.assign(astronaut,notify);notify=howtouse}}
